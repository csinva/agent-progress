...
1. Run `agent-progress ls --json` once for the state of every tracked job. (If
   `agent-progress` is not on PATH use
   `python3 ~/.claude/skills/agent-progress/scripts/agent_progress.py`.)
2. If `$ARGUMENTS` names an action (`done X`, `cancel X`, `rm X`), do it.
3. Otherwise report:
   - Run `agent-progress ls` and show the rendered bars verbatim in a code block.
...
4. **Check the monitors are actually working.** For any job showing no movement
   whose `monitor_kind` is not `time`, read `agent-progress log <id> -n 40` and
   either fix the monitor (`agent-progress update <id> --monitor … --glob …`) or say
   the job appears stalled.
5. If nothing is tracked, say so and offer `/agent-progress:track <command>`.
