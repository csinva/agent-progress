...
through (`agent-progress autotrack '<command>'` says which).
...
Follow the `agent-progress` skill. In short:
...
2. **Decide how this job's progress can be observed.** Read the script or
   ...
   `agent-progress monitors` lists them. This is the important decision; do not
